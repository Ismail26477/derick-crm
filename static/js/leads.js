...
let importAssignmentMode = "auto"
let importSelectedCallerId = null
let importSelectedCallerName = null
let allAvailableCallers = []
...
const AVAILABLE_FIELDS = [
  ...
  },
  {
    ...
    label: "Lead Category",
    required: false,
    description: "Lead category (Dubai Property, Australia Property, India Property, Loans)",
  },
]
  ...
  return [
    {
      ...
      leadCategory: "Dubai Property", // Sample data for leadCategory
    },
    {
      id: generateId(),
      name: "Sarah Johnson",
      ...
      leadCategory: "Australia Property", // Sample data for leadCategory
    },
    {
      id: generateId(),
      name: "Mike Chen",
      ...
      leadCategory: "India Property", // Sample data for leadCategory
    },
  ]
}
    ...
    .map(
      (lead, index) => `
        <tr>
            ...
            <!-- Added notes column with truncation to 100 characters, tooltip shows full notes -->
            <td title="${escapeHtml((lead.notes || "").substring(0, 150))}" style="max-width: 200px; word-break: break-word; white-space: normal; cursor: help;">
                ${lead.notes ? `<span style="font-size: 0.9rem; color: var(--text-secondary);">${escapeHtml((lead.notes || "").substring(0, 100))}${(lead.notes || "").length > 100 ? "..." : ""}</span>` : '<span style="color: #cbd5e1;">-</span>'}
            </td>
            <!-- Added Time and Category columns to table rendering -->
            <td><span class="category-badge" style="background: #e5e7eb; color: #374151; padding: 4px 8px; border-radius: 4px; font-size: 12px; font-weight: 500;">${escapeHtml(lead.leadCategory || "-")}</span></td>
...
function filterLeads() {
  ...
  const stageFilter = stageEl
    ? stageEl.value || (columnFilters.stage.length > 0 ? columnFilters.stage : "")
    : columnFilters.stage.length > 0
      ? columnFilters.stage
      : ""

  const sourceEl = document.getElementById("filterSource")
  const sourceFilter = sourceEl
    ? sourceEl.value || (columnFilters.source.length > 0 ? columnFilters.source : "")
    : columnFilters.source.length > 0
      ? columnFilters.source
      : ""

  const priorityEl = document.getElementById("filterPriority")
  const priorityFilter = priorityEl
    ? priorityEl.value || (columnFilters.priority.length > 0 ? columnFilters.priority : "")
    : columnFilters.priority.length > 0
      ? columnFilters.priority
      : ""

  const callerEl = document.getElementById("filterCaller")
  const callerFilter = callerEl
    ? callerEl.value || (columnFilters.assignedCaller.length > 0 ? columnFilters.assignedCaller : "")
    : columnFilters.assignedCaller.length > 0
      ? columnFilters.assignedCaller
      : ""

  const projectEl = document.getElementById("filterProject")
  const projectFilter = projectEl ? projectEl.value : ""

  const statusEl = document.getElementById("filterLeadStatus")
  const statusFilter = statusEl
    ? statusEl.value || (columnFilters.leadStatus.length > 0 ? columnFilters.leadStatus : "")
    : columnFilters.leadStatus.length > 0
      ? columnFilters.leadStatus
      : ""

  const nextFollowEl = document.getElementById("filterNextFollow")
  const nextFollowFilter = nextFollowEl ? nextFollowEl.value : ""

  const cityFilter = columnFilters.city.length > 0 ? columnFilters.city : ""
  const leadCategoryFilter = columnFilters.leadCategory.length > 0 ? columnFilters.leadCategory : ""
  ...
  filteredLeads = unfilteredLeads.filter((lead) => {
    const matchesSearch =
      ...
      (lead.notes && lead.notes.toLowerCase().includes(searchTerm)) // Added notes to search

    const matchesStage =
      !stageFilter || (Array.isArray(stageFilter) ? stageFilter.includes(lead.stage) : lead.stage === stageFilter)
    const matchesSource =
      !sourceFilter || (Array.isArray(sourceFilter) ? sourceFilter.includes(lead.source) : lead.source === sourceFilter)
    const matchesPriority =
      !priorityFilter ||
      (Array.isArray(priorityFilter) ? priorityFilter.includes(lead.priority) : lead.priority === priorityFilter)
    const matchesCaller =
      !callerFilter ||
      (Array.isArray(callerFilter)
        ? callerFilter.includes(lead.assignedCallerName)
        : lead.assignedCallerName === callerFilter)
    const matchesStatus =
      !statusFilter ||
      (Array.isArray(statusFilter) ? statusFilter.includes(lead.leadStatus) : lead.leadStatus === statusFilter)
    const matchesCity =
      !cityFilter || (Array.isArray(cityFilter) ? cityFilter.includes(lead.city) : lead.city === cityFilter)
    const matchesLeadCategory =
      !leadCategoryFilter ||
      (Array.isArray(leadCategoryFilter)
        ? leadCategoryFilter.includes(lead.leadCategory)
        : lead.leadCategory === leadCategoryFilter)
    ...
    return (
      ...
      matchesPriority &&
      matchesCaller &&
      (lead.project === projectFilter || !projectFilter) &&
      matchesStatus &&
      (lead.nextFollow === nextFollowFilter || !nextFollowFilter) &&
      matchesCity &&
      matchesLeadCategory // Include leadCategory check
    )
  })
...
function clearFilters() {
  ...
  document.getElementById("filterCity").value = "" // Clear city filter input
  document.getElementById("filterLeadCategory").value = ""
  // Clear column filters as well
...
function sortTable(column) {
  ...
  filteredLeads.sort((a, b) => {
    ...
    } else if (column === "createdAt" || column === "nextFollowDate" || column === "leadTime") {
      // Added leadTime to date sorting
      aVal = new Date(a.createdAt || a.nextFollowDate || a.leadTime)
      bVal = new Date(b.createdAt || b.nextFollowDate || b.leadTime)
    } else if (column === "notes") {
      // Added sorting for notes, though usually not sorted directly
      aVal = String(aVal || "").toLowerCase()
      bVal = String(bVal || "").toLowerCase()
    } else if (column === "leadCategory") {
      // Added sorting for leadCategory
      aVal = String(aVal || "").toLowerCase()
      bVal = String(bVal || "").toLowerCase()
    } else {
      aVal = String(aVal || "").toLowerCase()
...
function openImportModal() {
  ...
  // Reset assignment mode to default
  importAssignmentMode = "auto"
  importSelectedCallerId = null
  importSelectedCallerName = null
  document.getElementById("autoAssignRadio").checked = true
  document.getElementById("manualCallerDiv").style.display = "none"
...
function downloadTemplate() {
  const template = [
    [
      ...
      "Next Follow",
      "Next Follow Date",
      "Notes", // Added Notes to template
      "Lead Category", // Added Lead Category to template
    ],
    [
      "John Doe",
      "+1-555-0001",
      ...
      "2023-12-20T09:00:00Z", // Sample Lead Time
      "Dubai Property", // Sample Lead Category
    ],
    [
      "Jane Smith",
      "+1-555-0002",
      ...
      "Client seemed interested in the premium package.",
      "2023-12-19T11:30:00Z", // Sample Lead Time
      "Australia Property", // Sample Lead Category
    ],
  ]
  ...
  for (const fileHeader of fileHeaders) {
    ...
    for (const field of AVAILABLE_FIELDS) {
      ...
      if (
        ...
        (lowerHeader.includes("notes") && field.value === "notes") || // Added mapping for notes
        ...
        ((lowerHeader.includes("lead time") || lowerHeader.includes("received date")) && // Added mapping for leadTime
          field.value === "leadTime") ||
        ((lowerHeader.includes("lead category") || lowerHeader.includes("category")) && // Added mapping for leadCategory
          field.value === "leadCategory")
      ) {
    ...
    <div style="display: flex; gap: 8px;">
      ...
      <div style="font-size: 14px; color: #0c4a6e;">
        ...
        <ul style="margin-top: 8px; padding-left: 20px;">
          ...
          <li>Notes: -</li>
          <li>Lead Category: -</li> <!-- Added default for Lead Category -->
        </ul>
      </div>
    </div>
  `
...
function populateCategoryDropdown() {
  ...
  if (!categorySelect) {
    console.log("[v0] Category dropdown not found - element might not exist yet")
    return
  }

  // Get unique categories from all leads
  const categories = [...new Set(leads.map((l) => l.leadCategory).filter((c) => c && c.trim()))]
  categories.sort()

  // Reset to default option
  categorySelect.innerHTML = '<option value="">-- Select Category (Optional) --</option>'

  // Add hardcoded categories first if they don't exist in the list
  const hardcodedCategories = ["Dubai Property", "Australia Property", "India Property", "Loans"]
  const uniqueCategories = [...new Set([...hardcodedCategories, ...categories])]
  ...
  console.log("[v0] Category dropdown populated with:", uniqueCategories.length, "categories")
}
...
function goToPreview() {
  ...
  importedData = importedRawData.map((row) => {
    ...
    for (const [fileColumn, dbField] of Object.entries(columnMapping)) {
      ...
      } else if (dbField === "leadTime" && value) {
        // Parse leadTime
        const parsedLeadTime = new Date(value)
        mapped[dbField] = isNaN(parsedLeadTime.getTime()) ? null : parsedLeadTime.toISOString()
      } else if (value) {
    ...
    if (!mapped.notes) mapped.notes = "" // Default for notes
    ...
    if (!mapped.leadTime) mapped.leadTime = new Date().toISOString() // Default for leadTime

    if (selectedCategory) {
      mapped.leadCategory = selectedCategory
    } else {
      mapped.leadCategory = ""
    }

    if (importAssignmentMode === "manual" && importSelectedCallerId) {
      mapped.assignedCaller = importSelectedCallerId
      mapped.assignedCallerName = importSelectedCallerName
    } else if (importAssignmentMode === "auto") {
      // Will be assigned on server during import via round-robin
      mapped.assignedCaller = null
      mapped.assignedCallerName = null
    }

    return mapped
  })

  console.log("[v0] Mapped data ready:", importedData.length, "leads | Category applied:", selectedCategory || "none")
...
function displayImportPreview() {
  ...
  thead.innerHTML = `
    <tr>
      ...
      <th style="padding: 12px; text-align: left; font-weight: 600; border-bottom: 2px solid #e2e8f0;">Notes</th> <!-- Added Notes column -->
      <!-- Added Lead Time and Lead Category to preview -->
      <th style="padding: 12px; text-align: left; font-weight: 600; border-bottom: 2px solid #e2e8f0;">Lead Time</th>
      <th style="padding: 12px; text-align: left; font-weight: 600; border-bottom: 2px solid #e2e8f0;">Category</th>
    </tr>
  `
  ...
  tbody.innerHTML = importedData
    ...
    .map(
      ...
      <tr style="border-bottom: 1px solid #e2e8f0;">
        ...
        <td style="padding: 12px; max-width: 250px; word-break: break-word; white-space: normal;">${escapeHtml(lead.notes || "-")}</td> <!-- Added Notes column preview -->
        <!-- Added Lead Time and Lead Category to preview -->
        <td style="padding: 12px;">${lead.leadTime ? new Date(lead.leadTime).toLocaleString("en-IN", { year: "numeric", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" }) : "-"}</td>
        <td style="padding: 12px;">${escapeHtml(lead.leadCategory || "-")}</td>
      </tr>
    `,
    )
    .join("")
  ...
  if (importedData.length > 10) {
    tbody.innerHTML += `
      <tr style="background: #f8fafc;">
        <td colspan="11" style="padding: 12px; text-align: center; color: #64748b; font-size: 14px;"> <!-- Adjusted colspan -->
...
function confirmImport() {
  ...
  // Include assignment metadata with import
  const importPayload = {
    leads: importedData,
    assignmentMode: importAssignmentMode,
    selectedCallerId: importAssignmentMode === "manual" ? importSelectedCallerId : null,
    selectedCallerName: importAssignmentMode === "manual" ? importSelectedCallerName : null,
  }
  ...
  fetch("/api/leads/import", {
    ...
    .then((result) => {
      if (result && result.success) {
        const message =
          importAssignmentMode === "manual"
            ? `Successfully imported ${result.imported || importedData.length} leads assigned to ${importSelectedCallerName}!`
            : `Successfully imported ${result.imported || importedData.length} leads with auto-assignment!`

        showNotification(message, "success")
        addNotification(`${result.imported || importedData.length} leads imported`, "success", null)

        // Reset assignment variables
        importAssignmentMode = "auto"
        importSelectedCallerId = null
        importSelectedCallerName = null
...
function openAddLeadModal() {
  ...
  document.getElementById("leadNotes").value = "" // Clear notes field
  // <!-- Removed leadTime field reset -->
...
async function saveLead(event) {
  ...
  const leadFromForm = {
    ...
    leadCategory: (document.getElementById("leadCategory").value || "").trim(),
  }
...
function editLead(id) {
  ...
  if (lead.nextFollowDate) {
    ...
    document.getElementById("leadNextFollowDate").value = date.toISOString().slice(0, 16)
  } else {
    document.getElementById("leadNextFollowDate").value = ""
  }

  if (lead.leadTime) {
    const leadTimeDate = new Date(lead.leadTime)
    document.getElementById("leadTime").value = leadTimeDate.toISOString().slice(0, 16)
  } else {
    document.getElementById("leadTime").value = ""
  }

  document.getElementById("leadCategory").value = lead.leadCategory || ""
  document.getElementById("leadAssignedCaller").value = lead.assignedCaller || ""

  openLeadModal() // Call openLeadModal instead of directly manipulating classList
}
...
function viewLeadDetails(id) {
  ...
  document.getElementById("detailNextFollowDate").textContent = lead.nextFollowDate
    ...
    : "-"
  // <!-- Added notes field to details modal -->
  document.getElementById("detailNotes").textContent = escapeHtml(lead.notes || "No notes added")
  ...
  document.getElementById("detailLeadTime").textContent = lead.leadTime
    ? new Date(lead.leadTime).toLocaleString("en-IN", {
        year: "numeric",
        month: "short",
        day: "numeric",
        hour: "2-digit",
    ...
    : "-"
  document.getElementById("detailLeadCategory").textContent = escapeHtml(lead.leadCategory || "-")
...
function exportLeads() {
  const csv = [
    [
      ...
      "Stage",
      "Priority",
      "Notes", // Added Notes column to export
      "Project",
      "Lead Status",
      "Next Follow",
      "Next Follow Date",
  ...
  filteredLeads.forEach((lead) => {
    csv.push([
      ...
      lead.stage,
      lead.priority,
      lead.notes || "-", // Add notes to export
      lead.project || "-",
      lead.leadStatus || "-",
      lead.nextFollow || "-",
      lead.nextFollowDate
...
function toggleCallerAssignmentMode(mode) {
  importAssignmentMode = mode
  const manualDiv = document.getElementById("manualCallerDiv")

  if (mode === "manual") {
    manualDiv.style.display = "block"
    // Load callers for the dropdown
    loadCallersForImportDropdown()
  } else {
...
async function loadCallersForImportDropdown() {
  try {
    const res = await fetch("/api/callers", {
      credentials: "same-origin",
    })
    ...
    const data = await res.json()
    if (data.success && Array.isArray(data.callers)) {
      allAvailableCallers = data.callers.filter((c) => c.status === "Active")
      const select = document.getElementById("importCallerSelect")
      if (select) {
        select.innerHTML =
          '<option value="">-- Choose a Caller --</option>' +
          allAvailableCallers.map((c) => `<option value="${c.id}">${c.name || c.username}</option>`).join("")

        // Add change event listener
        select.onchange = function () {
          const selectedId = this.value
          const selected = allAvailableCallers.find((c) => c.id === selectedId)
          importSelectedCallerId = selectedId
          importSelectedCallerName = selected ? selected.name || selected.username : null
          console.log("[v0] Selected caller for import:", importSelectedCallerName)
        }
      }
    }
  } catch (err) {
    console.warn("Could not load callers for import:", err)
  }
}
...
function saveCallback() {
  ...
  // </CHANGE> Add notes field to lead object
  lead.notes = note
  ...
  // </CHANGE> Include notes in the persistLeadChange call
...
function toggleColumnFilter(event, column) {
  event.stopPropagation()
  const dropdown = document.getElementById(`filterDropdown-${column}`)

  // Close other dropdowns
  document.querySelectorAll(".column-filter-dropdown.active").forEach((el) => {
  ...
  if (!dropdown.getAttribute("data-populated")) {
    populateColumnFilterOptions(column)
    dropdown.setAttribute("data-populated", "true")
  }
}

function populateColumnFilterOptions(column) {
  const dropdown = document.getElementById(`filterDropdown-${column}`)
  const filterOptionsContainer = dropdown.querySelector(".filter-options")

  // Get unique values for the column
  let uniqueValues = []
  const deduped = deduplicateLeadsByPhone(leads)

  if (column === "assignedCaller") {
    uniqueValues = [...new Set(deduped.map((l) => l.assignedCallerName).filter((v) => v))]
  } else if (column === "source") {
    uniqueValues = [...new Set(deduped.map((l) => l.source).filter((v) => v))]
  } else if (column === "stage") {
    uniqueValues = [...new Set(deduped.map((l) => l.stage).filter((v) => v))]
  } else if (column === "priority") {
    uniqueValues = [...new Set(deduped.map((l) => l.priority).filter((v) => v))]
  } else if (column === "leadStatus") {
    uniqueValues = [...new Set(deduped.map((l) => l.leadStatus).filter((v) => v))]
  } else if (column === "city") {
    uniqueValues = [...new Set(deduped.map((l) => l.city).filter((v) => v))]
  } else if (column === "leadCategory") {
    // Added leadCategory to filter options
    uniqueValues = [...new Set(deduped.map((l) => l.leadCategory).filter((v) => v))]
  }
  ...
  filterOptionsContainer.innerHTML = uniqueValues
    .map(
      (value) => `
      <label class="filter-option" data-value="${escapeHtml(value)}">
        <input type="checkbox" value="${escapeHtml(value)}"
               onchange="updateColumnFilter('${column}', this)"
               ${columnFilters[column].includes(value) ? "checked" : ""}>
  ...
  const buttonsContainer = filterOptionsContainer.nextElementSibling
  if (!buttonsContainer || !buttonsContainer.classList.contains("filter-apply-btn")) {
    const buttonsHTML = `
      <div class="filter-apply-btn">
        <button class="btn-clear" onclick="clearColumnFilter('${column}')">Clear</button>
        <button class="btn-apply" onclick="applyColumnFilters()">Apply</button>
      </div>
    `
    filterOptionsContainer.insertAdjacentHTML("afterend", buttonsHTML)
  }
}

function updateColumnFilter(column, checkbox) {
  if (!checkbox || !checkbox.value) {
    console.log("[v0] Warning: checkbox is null or has no value")
    return
  }
  ...
  if (filterOption) {
    filterOption.classList.toggle("active", checkbox.checked)
  }
}
...
function clearColumnFilter(column) {
  ...
  const dropdown = document.getElementById(`filterDropdown-${column}`)
  if (dropdown) {
    dropdown.querySelectorAll('.filter-option input[type="checkbox"]').forEach((checkbox) => {
...
function applyColumnFilters() {
  console.log("[v0] Apply button clicked, columnFilters:", columnFilters)
  ...
  try {
    filterLeads()
    console.log("[v0] Filter applied successfully, filtered leads count:", filteredLeads.length)
  } catch (error) {
    console.error("[v0] Error in filterLeads:", error)
  }
}

function filterDropdownOptions(searchInput) {
  event.stopPropagation()
  const dropdown = searchInput.closest(".column-filter-dropdown")
...
document.addEventListener("click", (event) => {
  if (!event.target.closest(".filterable-column") && !event.target.closest(".column-filter-dropdown")) {
    document.querySelectorAll(".column-filter-dropdown.active").forEach((el) => {
      el.classList.remove("active")
    })
  }
